...
- Emit ``isso-postbox-rendered``, ``isso-comment-rendered`` and
  ``isso-thread-rendered`` DOM events so embedding pages can react to rendered
  markup. Add ``isso-postbox-submit``, ``isso-postbox-preview`` and
  ``isso-postbox-edit`` classes to the postbox buttons for easier restyling
  (`#1124`_, pkvach)
...
.. _#1124: https://github.com/isso-comments/isso/pull/1124
